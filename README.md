...
> 工作目录：`D:/atomcode/大学生软件/` · 版本：**v0.5**（文档流水线闭环 + 自学页对齐离线阅读器）· 更新：2026-09-23
...
当前基线：485 页 · 插图落位 296/296 · 表格还原 93/93 · 检索验收 9/9 · test_retrieval 16/16。
...
| 首页 · KPI 概览 + 课程卡 | 自学 · 章节树 + 教材原文内嵌插图（版式对齐离线阅读器） |
|---|---|
...
| `test_retrieval` | **16 / 16 通过**（含 7 条 **UI 路径回归锁**、1 条**命中词数回归锁**，见 §2.2） |
...
| 自学页版式 | **与离线阅读器 `data/reader.html` 同构**：顶部检索条 + 左栏 290px 章节树（书名分组金色）+ 右栏正文/结果卡片。课程下拉框、📌 知识点叶子、索引状态标签已按阅读器去掉；课程改由**首页课程卡**带入（`_goto_page("自学", 课程)`），引擎能力未删 |
| 检索结果 | **卡片列表**，与阅读器逐项对齐：命中章节卡 **≤6** 张（标题 + `书名 · 第 N 页` + 右上角 `命中 N 词`）、原文片段卡 **≤10** 张（BM25 分数 + 页码 + 前 **160 字**预览、命中词蓝底高亮）；点卡片跳该节原文并回选左栏。拒答时**一张卡都不给** |
| 章节阅读 | `QTextBrowser` 内嵌原文 + 插图 + 表格，**保序**呈现（图在原位，不堆章尾）；段落前带 `P<页码>` 徽章；插图点击 → **全屏遮罩放大**（点击任意处 / Esc 关闭） |
...
| 测试 | `tests/` **9 个测试文件全绿**（`test_retrieval` 16 · `test_materials_service` 11 · `test_study_service` 9 · `test_practice_service` 8 · `test_dashboard_service` 5 + 冒烟），且**不污染真实数据**（测试教材走独立课程名 + `finally` 清理） |
...
```
插图落位 296/296 · 表格结构 93/93 · 检索 9/9 · test_retrieval 16/16
```
...
1. **人工点检新版 exe** —— 项目根目录下的 `大学生软件.exe` 已于 2026-09-23 重打包（**105 MB**，含自学页对齐
   离线阅读器）。离屏实测可启动、`crash.log` 空，下一步是**肉眼验收**：把 `data/reader.html` 与软件并排打开，
   用同一批提问（「物理层」「三次握手」「量子纠缠」）逐项对照卡片张数、角标、预览字数、点卡跳转、遮罩开合；
   再确认正文插图在原位、表格渲染正确。
   > 自学页已按阅读器去掉课程下拉框，课程从**首页课程卡**点进去 —— 其余 3 门课接入后同样走这条入口。
