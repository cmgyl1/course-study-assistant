...
隔离说明：检索语料是"扫描 `data/textbooks_md/` 目录"得到的，所以测试教材
必须落盘才能被看板统计到。文件名带 `-测试教材` 后缀，且在 `finally` 里删除 ——
即使中途失败也不会残留污染真实语料（这是本测试曾经的缺陷）。
"""
import sys
...
def main() -> int:
    ...
    # 准备教材：**写盘即可** —— 检索语料按需从 Markdown 构建，没有"入库"这一步
    ...
    try:
        ...
        print(f"[OK] 看板: 教材片段={dash['course_chunks']}, 题库={dash['question_bank']}, "
